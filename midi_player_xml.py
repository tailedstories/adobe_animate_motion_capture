...
def my_smoothing(my_arr,mysigma):
    ...
    for x in range(len(my_arr)-1):
        # check if values are in range dist_low > x < dist_high
        if  abs(my_arr[x] - my_arr[x+1]) > dist_low and abs(my_arr[x] - my_arr[x+1]) < dist_high:
            # jerky movement correction, if velues are too different, replace with old +5
            # range to skip 
            if my_arr[x] >= val_high and my_arr[x+1] <= val_low or my_arr[x] <= val_low and my_arr[x+1] >= val_high:
                pass
            else:
                # add if biger value
        ...
        if my_arr[x] >= min_fix_high and my_arr[x+1] <= min_fix_low and abs(x-i) > 10 or my_arr[x+1] >= min_fix_high and my_arr[x] <= min_fix_low and abs(x-i) > 10:
            my_arr[i+1:x-1] = gaussian_filter1d(my_arr[i+1:x-1], sigma=mysigma)
            i=x+1
        # update index if values go between 127 and 0 a bunch
        if my_arr[x] >= freq_fix_high and my_arr[x+1] <= freq_fix_low or my_arr[x] <= freq_fix_low and my_arr[x+1] >= freq_fix_high:
            i=x+1
    # smoothing of the final  bin
    my_arr[i+1:x-2] = gaussian_filter1d(my_arr[i+1:x-2], sigma=mysigma)
    return my_arr 
...
with open('points.csv', newline='') as csvfile:
     ...
     for row in spamreader:
         ...
         if len(row) > 0:
             ...
             NearElbow = myAngle(my_n_elbow.get("X"),my_n_elbow.get("Y"),rot_my_elbow_n[0],rot_my_elbow_n[1])
             if send_360 == False:
                 NearElbow = int(remap(int(NearElbow), 0, 360, 0, 127))
             ...
             elif min_y - my_n_foot_line_1.get("Y") < 0 or min_y - my_n_foot_line_2.get("Y") >= 0 and min_y - my_n_foot_line_1.get("Y") < 0.01 or min_y - my_n_foot_line_2.get("Y") > 0 and min_y - my_n_foot_line_2.get("Y") < 0.01 :
             ...
             FarElbow = myAngle(my_f_elbow.get("X"),my_f_elbow.get("Y"),rot_my_elbow_f[0],rot_my_elbow_f[1])
             if send_360 == False:
                 FarElbow = int(remap(int(FarElbow), 0, 360, 0, 127))
             ...
             if min_y - my_f_foot_line_1.get("Y") < 0 or min_y - my_f_foot_line_2.get("Y") < 0:
                 ...
                 #print(foot_status)
             elif min_y - my_f_foot_line_1.get("Y") < 0 or min_y - my_f_foot_line_2.get("Y") > 0 and min_y - my_f_foot_line_1.get("Y") < 0.05 or min_y - my_f_foot_line_2.get("Y") > 0 and min_y - my_f_foot_line_2.get("Y") < 0.05 :
                 foot_f_status = "almost flying"
                 #print(foot_status)
             else:
        ...
        #########################################################
             ...
             #if len(sl_arr) > 1:
             #    c = parser.parse(row[0]) - time_memba
             #    sl_arr.append(c.total_seconds() * 1000)
             #else:
             #    sl_arr.append(0)
             #    time_memba = parser.parse(row[0])
...
if send_360 == False:
    ...
    for i in range(len(my_arr_NearShoulderScaling_s)):
            if my_arr_NearElbowScaling[i] > 350:
                my_arr_NearElbowScaling[i] = 360
